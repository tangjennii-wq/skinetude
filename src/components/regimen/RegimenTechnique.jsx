...
const RegimenTechnique = ({ onOpenLesson }) => {
  ...
  return (
    <div>
      <div className="text-sm font-light italic mb-6 max-w-2xl" style={{color:'var(--ink-soft)'}}>
        How you apply matters as much as what you apply. The pearls below cover the techniques that turn a shelf of products into an actual ritual.
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        {techniqueLessons.map(l => (
          <button key={l.id} onClick={() => onOpenLesson(l)} className="border p-6 text-left transition hover:border-[var(--ink)]" style={{background:'var(--cream-deep)', borderColor:'var(--line)'}}>
            ...
            <h3 className="font-serif text-xl italic leading-tight" style={{color:'var(--ink)'}}>{l.title}</h3>
            <p className="text-sm font-light italic mt-2 leading-relaxed" style={{color:'var(--ink-soft)'}}>{l.excerpt}</p>
          </button>
        ))}
      </div>
    </div>
